...
+(void)createTable{
    ...
    //本类不需要创建
    if([modelName isEqualToString:@"BaseModel"] || [modelName isEqualToString:@"OllaModel"]) {
        return;
    }
    ...
    //打印sql
//    DDLogInfo(@"%@",sql_Create);

    //执行创表语句
    BOOL createRes = [LLFMDB executeUpdate:sql_Create];
    
    if(!createRes){
//        DDLogInfo(@"%@ 创表失败",modelName);
        return;
    }
    DDLogInfo(@"表创建完毕%@",[NSThread currentThread]);
    //字段检查
...
+(void)fieldsCheck:(NSString *)table ivars:(NSArray *)ivars{
    ...
    //获取数据库字段信息
    NSArray *columns = [LLFMDB executeQueryColumnsInTable:table];
    
    //移除id字段
    NSMutableArray *columnsM=[NSMutableArray arrayWithArray:columns];
    [columnsM removeObject:@"id"];
    DDLogInfo(@"字段也检查完毕%@",[NSThread currentThread]);
    //数据库字段<->模型字段
    ...
    for (MJProperty *ivar in ivars) {
        ...
        //执行sql
        BOOL addRes = [LLFMDB executeUpdate:sql];
        
        if(!addRes){
            DDLogInfo(@"模型%@字段新增失败！",table);
            return;
        }
        
        DDLogInfo(@"注意：模型 %@ 有新增加的字段 %@,已经实时添加到数据库中！",table,ivar.name);
    }
